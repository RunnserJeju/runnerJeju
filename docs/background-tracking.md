...
## 5. 실시간 지도 경로 이어붙이기 (백그라운드 복귀) — 우려사항

**상태: 구현 완료, 실기기 검증됨.** 화면 끄고 걷다 켜면 실시간 지도에 **실제 걸은
경로**가 이어지고, 저장·서버 데이터도 정상인 것을 확인했다(2026-08-17).

### 5.1 무슨 문제였고 무엇을 고쳤나 (한 문단)

백그라운드에서 **데이터(`RunTracker._path`)는 이벤트 루프로 계속 쌓이지만**, 실시간
지도의 자라는 선은 **프레임 구동**이라 iOS가 화면을 끄면 멈춘다. 복귀하면 최신 점
하나만 들어와, 화면 끈 지점→켠 지점이 **직선**으로 그려졌다(데이터는 멀쩡, 표시만
착시). 고친 곳은 [`run_map_view.dart`](../frontend/app/lib/widgets/run_map_view.dart)의
`_syncLivePosition` 하나 — 두 위치 샘플의 `recordedAt` 시각차가 크면(백그라운드
복귀) 그 사이 `path` 점을 실제 경로대로 이어 그리고([`_fillLiveGap`]) 보간기를
비운다. 데이터 계층·서버·결과 화면은 **건드리지 않았다**.

### 5.2 버그 나면 여기부터 본다 (우선순위)

> **먼저 확인**: 실시간 지도만 이상한가, 저장/서버 데이터도 이상한가?
> **데이터가 정상이면 이 절(5장)의 문제**다 — 표시만 고치면 된다. 데이터까지
> 틀리면 1~2장(네이티브 백그라운드) 또는 [`run_tracker.dart`] 쪽이다.

1. **[가장 유력] 임계값 `_backgroundGapThreshold`(현재 3초).** gap이 **여전히
   직선**이면 이 값이 실제 백그라운드 복귀를 못 잡은 것 — 낮춘다. 반대로 평소
   러닝에서 멀쩡한 구간이 **울퉁불퉁 다시 그려지면** 너무 자주 걸리는 것 — 높인다.
   정상 갱신은 ≈300ms 간격이라 3초는 넉넉하지만, 기기·정확도에 따라 조정 여지.

2. **`recordedAt`가 없으면 gap 감지가 통째로 꺼진다.** `_hasBackgroundGap`은
   두 점의 `recordedAt`이 둘 다 있어야 동작한다. 실 GPS는 항상 채워지지만
   **시뮬레이션 위치원**([`simulated_location_service.dart`])이 시각을 안 넣으면
   gap을 못 채우고 직선으로 남는다. 시뮬레이션에서 재현 안 될 수 있음을 유의.

3. **채워진 구간이 살짝 거칠다(정상).** gap은 `path`(5m 게이트)에서 가져오고
   평소 라이브 선은 `currentPosition`(1m)이라, 백그라운드 구간만 해상도가 낮다.
   버그가 아니라 트레이드오프 — 촘촘하게 하려면 데이터원을 바꿔야 하는데 그럴
   값어치는 낮다.

4. **이중선·떨림이 보이면 `_interpolator.clear()` 누락을 의심.** fill 뒤 보간기를
   비워야 최신 점이 직전(백그라운드 전) 점에서 직선으로 다시 안 그어진다. 이
   `clear()`가 빠지거나 순서가 틀리면 실제 경로 위에 직선이 **겹쳐** 보인다.

5. **복귀 중 러닝이 초기화되면?** `_fillLiveGap`은 `await _renderIdle` 뒤
   `identical(_liveRoute, live)`·`_disposed`를 다시 확인하고 손을 뗀다
   ([`_breakLiveRoute`]와 같은 가드). 이 가드를 건드리면 "이미 걷어낸 오버레이를
   이어서 만지는" 크래시가 날 수 있으니 함께 유지.

### 5.3 회귀 체크 (수정으로 깨질 수 있는 유일한 것)

- [ ] **평소(화면 켠) 러닝 선이 예전처럼 매끄럽다.** — 게이트(3초) 덕에 정상
      갱신은 새 분기에 안 걸리지만, 동시성 렌더라 눈으로 한 번 확인.
- [ ] 백그라운드 복귀 시 **직선이 실제 경로로** 채워진다.
- [ ] 결과 화면·서버 경로는 예전과 동일(이 수정과 무관 — `currentPosition == null`
      이라 새 코드에 도달조차 안 함).
