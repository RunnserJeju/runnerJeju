...
- ~~**인증이 없다.**~~ 소셜 로그인(카카오/애플/구글)과 토큰 검증이 붙어 `current_user_id`가 실제 토큰에서 사용자를 꺼낸다. 코스 등록 API(`PUT /courses/gpx`)도 `require_admin`으로 막았다 — 관리자 전용 엔드포인트 목록과 그 배선 검사는 `tests/test_admin_routes.py`에 있다.
...
문제는 오히려 반대다 — **점 간격이 불균등하고 성기다**(평균 30m, 최장 252m). 그래서 등록 시 경로를 **15m 균등 간격으로 리샘플링해서** `courses.path`에 저장한다. 원본 GPX 점은 거리·고도·순환 계산에만 쓰고 DB에는 남지 않는다. 왜 균등해야 하는지, 검증 판정이 이 경로를 어떻게 쓰는지는 [verification.md](verification.md) 참고.
